...
def gameplay_video(vis_env, env_name, seed, 
                   q_networks, device, vid_dir, update_steps=None):
    ...
    for steps, agent in enumerate(vis_env.agent_iter()):
        if steps % len(vis_env.agents) == 0:
            frames.append(vis_env.render())

        observation, reward, termination, truncation, _ = vis_env.last()
        agent_handle = agent.split("_")[0]

        if termination or truncation:
            action = None
        elif random.random() < 0.05:  # 5% random actions, similar to atari
...
if __name__ == "__main__":
    ...
    gameplay_video(vis_env=vis_env, env_name=env_name, seed=1, q_networks=q_networks, device="cpu", vid_dir="video")