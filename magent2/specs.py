specs = {
    ...
    "combined_arms": {
        "agent_groups": {"redmelee", "redranged", "bluemelee", "blueranged"},
        "n_agents": 162,
        "action_shape": {
            "redmelee": 9,
            "redranged": 25,
...
for envs, spec in specs.items():
    for spec_name in ["action_shape", "observation_shape"]:
        if not isinstance(spec[spec_name], dict):
            spec[spec_name] = {
                handle: spec[spec_name] for handle in spec["agent_groups"]
            }
